...
def send_new_post_email(post_id, subscriber_email):
    ...
    post_url = f'http://127.0.0.1:8000{reverse("post_detail", args=[post.id])}'
...
def send_weekly_newsletter():
    ...
    html_content = render_to_string('news/email/weekly_news.html', {'posts': recent_posts})
    ...
    for subscriber in subscribers:
        try:
            ...
            logger.info(f'Письмо успешно отправлено на {subscriber.email}')
        except Exception as e:
            logger.error(f'Ошибка при отправке письма на {subscriber.email}: {e}')